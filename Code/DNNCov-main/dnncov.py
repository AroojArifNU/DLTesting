...
if (args.criteria[0]=='no-mcdc' or args.criteria[0]=='all'):
    ...
    subprocess.call("python3 ConstructTestSet.py -output_path {0}/test --{1} --input-rows {2} --input-cols {3} --input-channels {4}".format(args.outputs, dataset, args.img_rows, args.img_cols, args.img_channels), shell=True)
    subprocess.call("python3 Profile.py -model {0} -output_path {2}    --{1} --input-rows {3} --input-cols {4} --input-channels {5}".format(args.model, dataset, args.outputs, args.img_rows, args.img_cols, args.img_channels), shell=True)
    subprocess.call("python3 coverage_computer.py -model {1} -output_path {0} --{2} -i {0}/test".format(args.outputs, args.model, dataset), shell=True)
