...
The Testing section evaluates how accurately and confidently the  AI subsystem's predicts under various properties applied to images from each class. This phase focuses on directly measuring and quantifying the correctness of the  AI subsystem's.
 ...
 After generating test cases, measure the  AI subsystem's confidence for each class under each type of property.
    ...
    Global correctness evaluates the AI subsystem's performance on a combination of images, such as adding two digits. Given a set of 100 samples for each digit (0-9), we randomly select images to form digit pairs. The global correctness assesses the combined confidence for recognizing the result of operations like addition.
    
    Let \( x_1 \) and \( x_2 \) be two randomly selected images representing digits \( d_1 \) and \( d_2 \). The global correctness can be defined in both optimistic and pessimistic views as follows:
    
    \begin{itemize}
        \item \textbf{Optimistic Global Correctness}:
        \[
        \text{GL}_{opt} = \max(\text{Local Correctness}(x_1, T), \text{Local Correctness}(x_2, T))
        \]
        \item \textbf{Pessimistic Global Correctness}:
        \[
        \text{GL}_{pes} = \min(\text{Local Correctness}(x_1, T), \text{Local Correctness}(x_2, T))
        \]
    \end{itemize}
    
    For the addition operation, if we denote the sum of digits \( d_1 \) and \( d_2 \) as \( d_{\text{sum}} \), the global correctness for the operation can be formulated as:
    
    \[
    \text{Global Correctness} = f(x_1 + x_2) = \min(\text{GL}_{opt}, \text{GL}_{pes})
    \]
    
    Where the individual local correctness values for each transformation are considered to compute the final global correctness.
