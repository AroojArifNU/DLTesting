...
\begin{itemize}
    \item \textbf{Fast Gradient Sign Method (FGSM)}: Perturbs inputs by adjusting pixel values in the direction of the gradient, causing misclassifications with minimal changes.
    \item \textbf{Basic Iterative Method (BIM)}: Improves upon FGSM by applying it iteratively, leading to more effective perturbations.
    \item \textbf{Carlini and Wagner (C\&W) attack}: Uses optimization to find small perturbations that fool the network while maintaining high confidence in the incorrect predictions.
    \item \textbf{DeepFool}: Iteratively perturbs inputs to cross decision boundaries with minimal changes.
    \item \textbf{Jacobian-based Saliency Map Attack (JSMA)}: Selectively modifies input features to cause targeted misclassifications.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Translation}: Moves the entire image in one or more directions, testing the model's ability to handle shifts in object positioning.
    \item \textbf{Scaling}: Changes the size of the image, assessing the model's robustness to variations in object scale.
    \item \textbf{Shear}: Applies a shearing transformation, altering the shape of objects within the image to test model sensitivity to such distortions.
    \item \textbf{Rotation}: Rotates the image, challenging the model's ability to recognize objects from different orientations.
    \item \textbf{Brightness changes}: Alters the brightness of the image, testing the model's robustness to varying illumination.
    \item \textbf{Blurring}: Applies a blur effect to the image, assessing the model's ability to handle reduced clarity and sharpness.
\end{itemize}
...
% The properties that exist in the literature to verify real world scenario AI systems include:
% \begin{itemize}
%     \item \textbf{Noise}: Assessing performance under various types of noise (e.g., Gaussian noise, salt-and-pepper noise).
