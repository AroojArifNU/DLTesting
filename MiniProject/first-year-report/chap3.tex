...
The first component of this framework is formalized to specify model architecture $\mathcal{M}$, environmental properties, and input data characteristics $\mathcal{D}$. A pre-trained Convolutional Neural Network (CNN) $\mathcal{M}_{\text{CNN}}$ is  used, featuring a convolutional layer (32 filters, kernel size 3x3, ReLU activation function), a MaxPooling layer (2x2), followed by a flattened layer, a fully connected layer (128 neurons, ReLU), and an output layer with 10 neurons using softmax. The input data $\mathcal{D}$ consists of 10,000 test samples of MNIST images. The output labels are 10 classes representing digits from 0 to 9. Currently, specifications $\mathcal{S}$ are structured according to the experimental setup, which includes evaluating the model’s robustness under various conditions. Our specifications test all classes by default, but users can customize the classes to be tested based on their specific requirements.

For testing, different methods are employed based on the type of testing $\mathcal{T}$. In black-box testing $\mathcal{T}_{\text{black}}$, semantic adversarial properties $\mathcal{P}_{\text{sem}}$ such as rotation, brightness adjustment, blur, shear, and contrast modification are made using Python libraries like cv2 and ImageDataGenerator, are applied by adjusting the input data $\mathcal{D}$ without knowledge of the model's internal details. In white-box testing $\mathcal{T}_{\text{white}}$, adversarial attacks $\mathcal{P}_{\text{adv}}$ are used by accessing the model's internal structure and parameters. Adversarial examples are generated using the Foolbox library with methods like the Fast Gradient Sign Method (FGSM), Projected Gradient Descent (PGD), AdamPGD, and Deepfool are used to create these examples by computing gradients



\textbf{Note:} Grey-box testing $\mathcal{T}_{\text{grey}}$, involving partial model access, will be explored in future work.

\begin{example}
  \label{ex:mnist-adder}
  An instance of a simple AI system is an \emph{MNIST Digit Adder} $\mathcal{S}_{\text{adder}} = (\mathcal{F}, \mathcal{D})$. In this system, $\mathcal{F}$ consists of a DNN component $\mathcal{M}_{\text{CNN}}$, The dataset $\mathcal{D}$ contains MNIST digits (0 to 9), and a software component $\omega$ that performs the addition.

  Formally, we define $\mathcal{F} = (\{\mathcal{M}_{\text{CNN}}\}, \omega)$, where $\omega$ integrates the output of the DNN. The function $\mathcal{M}_{\text{CNN}}$ takes single MNIST digit images as input and recognizes the digits. The software component $\omega$ can then pick any two recognized digits and compute their sum.

  For example, $\mathcal{S}_{\text{adder}}$ can perform the task of digit addition:
  \begin{equation}
    \mathcal{S}_{\text{adder}}(x_1, x_2) = \omega(\mathcal{M}_{\text{CNN}}(x_1), \mathcal{M}_{\text{CNN}}(x_2)),
  \end{equation}
  where $\omega(a, b) = a + b$.

  Given two MNIST digit images $x_1$ and $x_2$, $\mathcal{S}_{\text{adder}}$ recognizes the digits and computes their sum.
\end{example}

\section{Sampling}
The sampling process is designed to identify efficient and corner cases by employing a hybrid approach that combines Borderline-SMOTE and ADASYN using a Scikit-learn pipeline. This method generates synthetic examples, enhancing class balance and model robustness. Borderline-SMOTE focuses on creating samples near decision boundaries, while ADASYN targets hard-to-learn examples, ensuring that both typical and challenging cases are effectively covered.

Data is sampled according to the requirements specified in the specifications. By default, all classes are tested, but users can customize the classes to be sampled based on specific requirements. The hybrid approach is applied as follows for each classifier $f_i$ independently:


The full sample $\mathcal{S}_i$ for classifier $f_i$, $i=1,\dots,n$, is computed by directly applying the hybrid sampling method to the initial dataset $\mathcal{D}$:

\begin{equation}
\mathcal{S}_i = \text{HybridSampling}(\mathcal{D})
\end{equation}

where the HybridSampling method is defined as the combination of Borderline-SMOTE and ADASYN:


\begin{example}
  \label{ex:sampling}
  To further illustrate the sampling process, consider the classifier $\mathcal{M}_{\text{CNN}}$ and $\mathcal{D}$ from Example.\ref{ex:mnist-adder}. For each class $c$, we randomly sample an equal number of correctly classified instances to form the initial subset $\mathcal{S}_{\text{CNN}}^c$. This subset is then enhanced by applying the hybrid method using a Scikit-learn pipeline, which combines Borderline-SMOTE and ADASYN to generate synthetic samples. The final sample $\mathcal{S}_{\text{CNN}}$ for testing is obtained as:

  \begin{equation}
    \mathcal{S}_{\text{CNN}} = \text{HybridSampling}(\mathcal{S}_{\text{CNN}}^c)
  \end{equation}

  where the HybridSampling method is defined as the combination of Borderline-SMOTE and ADASYN:

%  $\mathcal{S}_{\text{CNN}}^c = \mathcal{S}_{\text{CNN}}^c \cup \text{Borderline-SMOTE}(\mathcal{S}_{\text{CNN}}^c) \cup \text{ADASYN}(\mathcal{S}_{\text{CNN}}^c)$

%   This process ensures a balanced and comprehensive set of test samples, covering both typical and challenging cases.
\end{example}
